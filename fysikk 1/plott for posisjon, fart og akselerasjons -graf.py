...
#hvis du må redigere f.eks komma til punktum i en liste med data kan du bruke "tekstredigering" (søk i spotlight), trykk på en random fil.
# Deretter lim inn dataen. deretter trykk på rediger oppe i "kontroll linjen" (øverst på mac skjermen), trykk på finn, søk og erstatt. voila!

data = np.loadtxt("fjærvogn_copy.txt")

t = data [:,0]  #alle verdier i rad nr 0
s = data[:,1]   #alle verdier i rad nr 1
...
v = np.zeros(n-di)                     #lager en liste med nuller
...
di = di+dia
a = np.zeros(n-di)   #range må være n-2, n-1 går ikke, fordi
for i in range(di,n-di):
